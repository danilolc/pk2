...
int Draw_Intro(){
	...
	DWORD sdl_alku        = kaantaja_loppu + 100;
	DWORD sdl_loppu       = sdl_alku + 400;
	...
	if (introlaskuri > tekijat_alku) {
		CreditsText_Draw(tekstit->Get_Text(PK_txt.intro_a_game_by),fontti1, 120, 230, tekijat_alku, tekijat_loppu, introlaskuri);
		CreditsText_Draw("janne kivilahti",			fontti1, 120, 250, tekijat_alku+20, tekijat_loppu+20, introlaskuri);
		CreditsText_Draw(tekstit->Get_Text(PK_txt.intro_original), fontti1, 120, 275, tekijat_alku+40, tekijat_loppu+40, introlaskuri);
		CreditsText_Draw("antti suuronen 1998",		fontti1, 120, 295, tekijat_alku+50, tekijat_loppu+50, introlaskuri);
	}
	...
	if (introlaskuri > testaajat_alku) {
		...
		CreditsText_Draw("juho rytkönen",				fontti1, 120, 270, testaajat_alku+30, testaajat_loppu+30, introlaskuri);
	...
	if (introlaskuri > sdl_alku) {
		CreditsText_Draw("powered by", fontti1, 120, 230, sdl_alku, sdl_loppu, introlaskuri);
		CreditsText_Draw("SDL 2.0",  fontti1, 120, 250, sdl_alku+20, sdl_loppu+20, introlaskuri);
	}
