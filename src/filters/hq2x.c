...
HQX_API void HQX_CALLCONV hq2x_32( const uint8_t * sp, uint32_t srb, uint32_t * dp, uint32_t drb,
    const uint32_t * palette, int Xres, int Yres )
{
    ...
    for (j=0; j < Yres; j++)
    {
        ...
        for (i=0; i<Xres; i++)
        {
            w[2] = palette[*(sp + prevline)];
            w[5] = palette[*sp];
            w[8] = palette[*(sp + nextline)];

            if (i>0)
            {
                w[1] = palette[*(sp + prevline - 1)];
                w[4] = palette[*(sp - 1)];
                w[7] = palette[*(sp + nextline - 1)];
            }
            else
            {
                w[1] = w[2];
            ...
            if (i<Xres-1)
            {
                w[3] = palette[*(sp + prevline + 1)];
                w[6] = palette[*(sp + 1)];
                w[9] = palette[*(sp + nextline + 1)];
            }
            else
            {
                w[3] = w[2];
