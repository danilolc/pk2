					 ...
					 int max_colors)
{
	...
	PD_ruudun_bpp = bpp;
  PD_buffers[PD_TAUSTABUFFER] = SDL_SetVideoMode(leveys, korkeus, bpp, SDL_HWSURFACE|SDL_ANYFORMAT|SDL_SRCCOLORKEY|SDL_DOUBLEBUF);
...
{
	int i = PD_buffereita_varattu, varaus;
...
int PisteDraw_Lataa_Kuva(int index, char *filename, bool lataa_paletti)
{
  ...
  if(!PD_buffers[index])
  {
	...
	if(lataa_paletti) {
		Uint32 transcolor = 255;
		if(PD_buffers[index]->format->BitsPerPixel > 8)
			transcolor = SDL_MapRGB(PD_buffers[index]->format, 155,232,224);
  	SDL_SetColorKey(PD_buffers[index], SDL_SRCCOLORKEY|SDL_RLEACCEL, transcolor);
	}
