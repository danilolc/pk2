...
int PisteFont::Get_bitmap(int buffer_x, int buffer_y, int ruudun_leveys, int buffer_index)
{
	...
	PisteDraw_Buffer_Flip_Nopea(buffer_index,font_buffer,0,0,buffer_x,buffer_y,font_bitmap_leveys-1,font_korkeus);
}
...
int PisteFont::Piirra_merkkijono_lapinakyva(int font_x, int font_y, int lPitch, SDL_Rect klipperi, char *merkkijono,
	...
	while (merkki != '\0')
	{
		...
		if (ix > -1)
		{
			for (x=0;x<font_leveys;x++)
			{
				...
				if (fx >= klipperi.x && fx <= klipperi.x+klipperi.w)
					for (y=0;y<font_korkeus;y++)
					{
						...
						if (color!=255)
						{
							...
							if (fy >= klipperi.y && fy < klipperi.y+klipperi.h)
							{
								//fy %= 470;
								fy *= lPitch;
								fy += fx;
